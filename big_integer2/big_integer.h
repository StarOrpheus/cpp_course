...
public:
    ...
    size_t length() const;
    big_int_storage get_data() const;

    friend big_integer apply_with_complement(big_integer const& x,
                                             big_integer const& y,
                                             std::function<uint32_t(uint32_t, uint32_t)> F);
    friend big_int_storage get_complement_data(big_integer const& x);
    friend big_integer get_decomplement_data(big_int_storage const& x);
